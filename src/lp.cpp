...
void lp_add_row(LinearProgram *lp, const int nz, const int *indexes, const double *coefs, const char *name, const char sense, const double rhs)
{
    ...
    cpxError = CPXaddrows(LPcpxDefaultEnv, lp->cpxLP, 0, 1, nz, &rhs, &uSense, matBeg, indexes, coefs, NULL, (char **) &name);
    lp_check_for_cpx_error(LPcpxDefaultEnv, cpxError, __FILE__, __LINE__);
    ...
    int grbError = GRBaddconstr( lp->lp, nz, indexes, coefs, uSense, rhs, name );
    lp_check_for_grb_error( LPgrbDefaultEnv, grbError, __FILE__, __LINE__ );
#endif
#ifdef GLPK
...
char lp_isMIP(const LinearProgram *lp)
{
...
int lp_num_binary_cols( const LinearProgram *lp )
{
    ...
    for(int i = 0; i < lp_cols(lp); i++)
    {
        const double lb = lp_col_lb(lp, i);
        const double ub = lp_col_ub(lp, i);
        if( (lp_is_integer( lp, i )) && ( fabs( lb ) <= EPS ) && (fabs( ub - 1.0 ) <= EPS) )
            binaries++;
    }
    return binaries;
}
...
int lp_generate_clique_cuts(LinearProgram *lp, const CGraph *cg, CutPool *cutPool, int argc, const char **argv)
{
    ...
    fill_x(lp, x);
    if(clq_sep_get_extend_method(sep) == CLQEM_PRIORITY_GREEDY || clq_sep_get_extend_method(sep) == CLQEM_EXACT)
    {
        fill_rc(lp, rc);
