      ...
      do
         ...
         if [ $useClique = "1" ]; then
            if [ $useOtherCuts = "0" ]; then
               cat "$probName-log-dbound-----$maxDepth-$minViol-$maxPasses.txt" | uniq > u"$probName-log-dbound-----$maxDepth-$minViol-$maxPasses.txt"

               logFNNoCliques="u$probName-log-dbound-----$maxDepth-$minViol-$maxPasses.txt"
